...
def p_claimed_boot_amount(params, substep, state_history, previous_state):
    if previous_state['timestep'] + 1 < params['days_for_gift_activation']:
        delta_claimed_boot_amount = params['claimed_at_activation_share'] * params['boot_gift_amount_init'] / \
                                    params['days_for_gift_activation']
    else:
        delta_claimed_boot_amount = (1 - params['claimed_at_activation_share']) * params['boot_gift_amount_init'] / \
...
def p_capitalization_per_agent(params, substep, state_history, previous_state):
    delta_capitalization_per_agent = - 0.7 * (params['start_capitalization_per_agent'] * math.pow(params['agents_count_at_activation'], 0.7)) / \
...
def p_cyberlinks(params, substep, state_history, previous_state):
    cyberlinks = 2.7 * math.pow(previous_state['agents_count'], 0.3) + params['extra_links'] + params['guaranted_links']
